...
export async function readArtifactForUser(userId: string, artifactId: string, deps: { env?: Env; redis?: RedisLike; gates?: readonly LaunchGate[] } = {}): Promise<ArtifactReadResult> {
  const env = deps.env ?? process.env;
  try {
    if (!isGloballyEnabled(env) || !AI_ROUTE_IDS.some((route) => isRouteEnabled(route, env, { gates: deps.gates }))) return { status: 'unavailable', reason: 'FLAG_OFF' };
