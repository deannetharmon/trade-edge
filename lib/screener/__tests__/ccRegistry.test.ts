...
describe('registry structure: every criterion declares what it is', () => {
  ...
  it('the width is an editable dollar setting (unlike the fixed CSP policy) and says so', () => {
    const width = getCcCriterion('width');
    expect(width.fixed).toBe(false);
    expect(width.unit).toBe('$ per share');
    expect(width.control.kind === 'rule' && width.control.key).toBe('BID_ASK_MAX');
  });
  ...
  it('cards group the criteria in the order the modal renders them, with the receipt-only line left out', () => {
    ...
    expect(ccCriteriaForCard('search').map((c) => c.id)).toEqual(['dte', 'delta', 'width']);
...
describe('receipts are built from the registry', () => {
  it('groups the search range, always-applied limits, advisory, and capacity', () => {
    ...
    expect(groups.search.items).toEqual(['21–45 DTE', 'Δ 0.20–0.35', 'width ≤ $0.20']);
  ...
  it('reflects the configured rules', () => {
    const groups = byKey(buildCcReceipt(values({ rules: { ...DEFAULT_CC_RULES, DTE_MIN: 14, DTE_MAX: 21, BID_ASK_MAX: 0.1, OI_MIN: 500 } })));
    expect(groups.search.items).toEqual(['14–21 DTE', 'Δ 0.20–0.35', 'width ≤ $0.10']);
