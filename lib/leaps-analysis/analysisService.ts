...
/** Display precision for the snapshot the model reads: money/percent 2 dp, delta and IV 4 dp (broker floats such as 12.692000000000007 are noise). */
const roundTo = (value: number | null, digits: number): number | null => (value == null || !Number.isFinite(value) ? value : Math.round(value * 10 ** digits) / 10 ** digits);
export function buildSnapshot(review: ServerLeapsReview, intent: LeapsIntent, quantity: number, objective: string, criteria: SnapshotCriteria | null = null): Snapshot {
  ...
  const rawMechanics = { multiplier: review.multiplier, midPerShare: mid, costPerContract: mid == null ? null : mid * review.multiplier, totalEstimatedCost: mid == null ? null : mid * review.multiplier * quantity, intrinsicPerShare: intrinsic, extrinsicPerShare: extrinsic, extrinsicPctOfMidCost: mid && extrinsic != null ? extrinsic / mid * 100 : null, breakeven: mid == null ? null : review.strike + mid, breakevenPctAboveSpot: mid != null && review.spot ? ((review.strike + mid - review.spot) / review.spot) * 100 : null, spreadPct: review.qualification.spreadPct };
  const mechanics = Object.fromEntries(Object.entries(rawMechanics).map(([key, value]) => [key, key === 'multiplier' ? value : roundTo(value, 2)])) as Record<string, number | null>;
  const contract = { occSymbol: review.occSymbol, symbol: review.symbol, strike: review.strike, expiration: review.expiration, dte: review.dte, bid: review.bid, ask: review.ask, spot: review.spot, delta: roundTo(review.delta, 4), openInterest: review.openInterest, impliedVolatility: roundTo(review.impliedVolatility, 4), optionQuoteTimestamp: review.optionQuoteTimestamp, underlyingQuoteTimestamp: review.underlyingQuoteTimestamp, instrumentType: review.instrumentType, multiplier: review.multiplier, provider: review.provider, fetchedAt: review.fetchedAt, marketSession: review.marketSession ?? 'unknown', quoteBasis: review.quoteBasis ?? 'live' };
  const unsigned = { version: LEAPS_ANALYSIS_SNAPSHOT_VERSION, intent, quantity, objective, contract, criteria, qualification: { ...review.qualification, extrinsicPctOfCost: roundTo(review.qualification.extrinsicPctOfCost, 2), spreadPct: roundTo(review.qualification.spreadPct, 2) }, mechanics, provenance: { provider: review.provider, policyVersion: review.qualification.policyVersion, analysisLookupAt: review.fetchedAt, freshnessPolicy: 'analysis-market-aware-v1' } };
