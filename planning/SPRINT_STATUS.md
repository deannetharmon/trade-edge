...
**Last Updated:** 2026-07-24 (DOC-0001 — Project Documentation & Repository Reconciliation. Reconciled this document against Git history after the successful completion and merge of TC-0001, PT-0002A, PT-0002B, DT-0001, OE-0001 [previously undocumented as merged here], and OE-0002A. `main` and `origin/main` are both at `7acb641` ("merge: OE-0002A opportunity engine activation"), working tree clean. **DOC-0001 is the only active sprint.**)
**Primary Branch:** `main`
...
**DOC-0001 — Project Documentation & Repository Reconciliation** is the **current active sprint**, in progress on `feature/doc-0001-project-reconciliation` (created off `main` @ `7acb641`). Documentation-only: no application code, tests, configuration, dependencies, or runtime behavior changed. Scope: reconcile `planning/SPRINT_STATUS.md` (this document), `docs/roadmap/ROADMAP.md`, `docs/HANDOFF.md`, `planning/PROJECT_GOVERNANCE.md`, and the design/review documents for TC-0001, PT-0002A, PT-0002B, DT-0001, OE-0001, and OE-0002A against verified Git history.

**Every sprint through OE-0002A listed above is accepted, complete, and merged into `main`.** No implementation sprint is currently active — only DOC-0001's documentation reconciliation. **OE-0002B (Dashboard Integration)** and **OE-0003 (Optional Opportunity Context)** are queued as planned future work (see Known Follow-Ups and `docs/roadmap/ROADMAP.md`); neither is approved, scoped, or started. PI-0015 / Portfolio Intelligence corrections remain queued for live-market acceptance validation. `epic/autopilot` remains untouched by every sprint in this document.
...
Verified 2026-07-24 (DOC-0001, current):
...
- **OE-0002B — Dashboard Integration (planned, not started):** decide whether `/dashboard`'s `BestOpportunityCard` (currently still passing a hardcoded empty `DecisionAnalysis[]`) should share OE-0002A's real Screener-sourced feed, and if so, how that feed reaches an already-LIVE-gated surface without introducing a new persistence layer.
- **OE-0003 — Optional Opportunity Context (planned, not started):** wire real, portfolio-mode-gated capital/exposure data into `OpportunityContext` (currently `availableCapital: 0`, no exposure fields, by deliberate OE-0002A design). Accepted by Quinn and Paul as a future architectural enhancement, not part of OE-0002A. Contingent on resolving `/screener`'s PortfolioMode gating question (see below).
...
**Every sprint through OE-0002A is accepted, complete, and merged.** DOC-0001 (documentation reconciliation, this document) is the only active work. No implementation sprint is currently selected as "next" — that determination belongs to the Product Owner, guided by the candidates below (not ranked; ranking is a Product Owner decision):

1. **OE-0002B — Dashboard Integration** — decide whether/how `/dashboard`'s Best Opportunity card shares OE-0002A's real feed.
