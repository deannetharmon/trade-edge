...
**Last Updated:** 2026-07-25 (WA-0004 merged. `main` and `origin/main` are both at `f0b884d` ("feat(portfolio): implement WA-0004 briefing separation"), working tree clean on `main`. **WA-0005's CES has been accepted and frozen by the Product Owner; implementation is now authorized.**)
**Primary Branch:** `main`
...
**Every sprint through WA-0004 listed above is accepted, complete, and merged into `main`.** **WA-0005 — Opportunities Workspace**'s CES (`docs/design/WA-0005-Opportunities-Workspace-CES.md`, branch `feature/wa-0005-opportunities-workspace-design`) has been **accepted and frozen by the Product Owner; implementation is now authorized** against its §20 Acceptance Criteria and §26 Proposed Implementation Sequence, without further design escalation. The CES went through three Product Owner review rounds before acceptance: the first returned it for internal-consistency corrections (a missing Acceptance Criteria section, a self-contradictory "no qualifying opportunities" contract, a self-contradictory candidate-detail tier contract with an unverified field-passthrough claim, an unspecified staleness threshold, and a citation of the `#best-opportunity` anchor as though it existed on `/screener` when repository verification shows it exists only on Mission Control's own section) and also resolved, as two now-binding Product Owner rulings recorded directly in the CES (§10): (1) available capital stays hardcoded `0` on `/screener` — unchanged — with a new required, persistent capital-limitation notice and canonical integration deferred to the existing **OE-0003** ticket; (2) `/screener`'s information hierarchy is reordered to scan controls → Ranked Opportunities → "All Scan Results," with all existing scan mechanics preserved. A second, narrower round resolved a remaining internal inconsistency: states 2 and 5 of the §15 state contract were made mutually exclusive via the `rawAnalyses.length` discriminator, §22's test-plan ownership was split correctly between `BestOpportunitiesPanel` component tests and a new `/screener` page/integration test suite, and §14 was corrected to remove ambiguity around new sorting/filter controls — WA-0005 adds no new Ranked Opportunities sorting or disposition-filter UI, and every `REJECTED` candidate remains visible by default. No further corrections remain outstanding; implementation may now proceed. **WA-0006** (Legacy Priority List retirement) remains queued after WA-0005, not approved, scoped, or started. **OE-0003 (Optional Opportunity Context)** is queued as planned future work (see Known Follow-Ups and `docs/roadmap/ROADMAP.md`) — now also the named follow-on for WA-0005's Ruling 1 — not approved, scoped, or started. The MB-0001B follow-up pass (page narrative refactor and UI concepts) is not approved, scoped, or started. PI-0015 / Portfolio Intelligence corrections remain queued for live-market acceptance validation. `epic/autopilot` remains untouched by every sprint in this document.
...
**Every sprint through WA-0004 is accepted, complete, and merged.** **WA-0005 — Opportunities Workspace's CES has been accepted and frozen by the Product Owner** (`docs/design/WA-0005-Opportunities-Workspace-CES.md`); **implementation is authorized** against its §20 Acceptance Criteria and §26 Proposed Implementation Sequence. The two open Product Owner decisions the original draft left open (available-capital wiring, `/screener` information hierarchy) are resolved as binding rulings recorded in the CES itself (§10) — capital stays `0` with a new required limitation notice and deferral to **OE-0003**; the page reorders to scan controls → Ranked Opportunities → "All Scan Results." Once WA-0005 is implemented and completed, the remaining candidates below stay queued (not ranked; ranking is a Product Owner decision):
