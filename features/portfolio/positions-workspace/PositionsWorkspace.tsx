...
function AnalysisRow({ onIntentChange, position: p, columns, th, actions, onExecute, renderStopControl, onAnalyze, savedNote, onSaveNote, savedAlert, onSaveAlert, chartOpen, setChartOpen, sparkData, setSparkData, sparkLoading, setSparkLoading }: { onIntentChange?: (key: string, intent: PositionIntent) => void; position: Position; columns: AnalysisColumnId[]; th: typeof THEMES[Theme]; actions: ActionType[]; onExecute?: (position: Position, action: ActionType, initialRollMode?: 'close' | 'roll') => void; renderStopControl?: (position: Position) => ReactNode; onAnalyze?: (position: Position) => void; savedNote: string; onSaveNote: (position: Position, note: string) => Promise<void>; savedAlert: { targetPrice: number; direction: 'above' | 'below' } | null; onSaveAlert: (position: Position, targetPrice: number | null, direction: 'above' | 'below') => Promise<void>; chartOpen: boolean; setChartOpen: (open: boolean) => void; sparkData: number[] | null; setSparkData: (data: number[] | null) => void; sparkLoading: boolean; setSparkLoading: (loading: boolean) => void }) {
  ...
  const cell: Record<AnalysisColumnId, ReactNode> = {
    ...
    evolution: <><span className="block text-white">first tracked → now</span><SemanticComparison label="P/L" prior={firstPnl} current={pnl} tone={comparisonTone(firstPnl, pnl)} digits={0} />{valueSplit && <><SemanticComparison label="Int" prior={valueSplit.intrinsic.atEntry} current={valueSplit.intrinsic.now} tone={intrinsicTone({ split: valueSplit, intent: p.intent, loneShort })} digits={2} /><SemanticComparison label="Ext" prior={valueSplit.extrinsic.atEntry} current={valueSplit.extrinsic.now} tone={extrinsicTone(valueSplit)} digits={2} /></>}<SemanticComparison label="Δ" prior={first?.netDelta ?? p.deltaAtEntry} current={p.netDelta} tone="neutral" /><SemanticComparison label="Θ" prior={first?.theta ?? p.thetaAtEntry} current={p.theta} tone={comparisonTone(first?.theta ?? p.thetaAtEntry, p.theta, true)} /><SemanticComparison label="Γ" prior={first?.gamma ?? p.gammaAtEntry} current={p.gamma} tone={comparisonTone(first?.gamma ?? p.gammaAtEntry, p.gamma, false)} digits={3} /><SemanticComparison label="V" prior={first?.netVega ?? p.vegaAtEntry} current={p.netVega} tone="neutral" /><SemanticComparison label="IV" prior={first?.iv ?? p.ivAtEntry} current={p.iv} tone={ivTone(first?.iv ?? p.ivAtEntry, p.iv, p.netVega)} suffix="%" /><SemanticComparison label="IVR" prior={first?.ivr ?? p.ivrAtEntry} current={p.ivr} tone="neutral" /></>,
