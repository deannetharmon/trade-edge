...
function AnalysisView({ model, th, getManagementActions, onExecute, renderStopControl, onAnalyze, renderAnalysisConversation, onFindPmccShortCall }: { model: PositionsWorkspaceModel; th: typeof THEMES[Theme] } & ManagementActionProps) {
  ...
  return <>
    <section aria-label="Existing-position income eligibility" className={`mb-3 rounded-xl border ${th.border} p-3`}>
      ...
      {incomeSectionOpen && <div id="existing-position-income-content"><div className="mt-3 grid gap-2 lg:grid-cols-2">{(model.incomeOpportunities ?? []).map(opportunity => <div key={opportunity.id} className="rounded border border-white/10 p-3 text-xs"><div className="flex items-start justify-between gap-2"><div><b className="text-white">{opportunity.symbol} · {opportunity.title}</b><p className={`mt-1 ${opportunity.status === 'eligible' ? 'text-emerald-300' : opportunity.status === 'no-capacity' ? 'text-amber-300' : opportunity.status === 'unavailable' ? 'text-red-300' : th.textMuted}`}>{opportunity.status === 'eligible' ? 'Ready — find short call' : opportunity.status === 'no-capacity' ? 'Short-call capacity unavailable' : opportunity.status === 'unavailable' ? 'Broker data unavailable' : 'Not eligible'}</p></div>{opportunity.status === 'eligible' && opportunity.kind === 'pmcc-short-call' && <button type="button" onClick={() => onFindPmccShortCall?.(opportunity)} className="min-h-8 rounded border border-teal-500/50 px-2 text-[10px] text-teal-300 focus:ring-2 focus:ring-teal-400">Find short call</button>}</div><p className={`mt-2 ${th.textFaint}`}>{opportunity.reason}</p><p className="mt-1 text-[10px] text-cyan-200"><b>Next:</b> {opportunity.nextStep}</p>{opportunity.kind === 'covered-call' && <p className={`mt-2 ${th.textFaint}`}>Shares {opportunity.sharesOwned ?? '—'} · Allocated {opportunity.allocatedContracts ?? '—'} · Reserved {opportunity.reservedContracts ?? '—'} · Available {opportunity.availableContracts ?? '—'}</p>}<p className={`mt-2 text-[10px] ${th.textFaint}`}>Freshness: {opportunity.freshness}</p></div>)}</div>
