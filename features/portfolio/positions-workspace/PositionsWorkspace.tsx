...
import { buildExtrinsicView } from './model/extrinsic';
...
function PortfolioView({ groups, th, renderStopControl, onIntentChange }: { groups: SymbolGroupViewModel[]; th: typeof THEMES[Theme]; renderStopControl?: (position: Position) => ReactNode; onIntentChange?: (key: string, intent: PositionIntent) => void }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,38%)]">
    ...
    {group && <SymbolDetail onIntentChange={onIntentChange} group={group} th={th} mobile={mobileDetail} onBack={() => setMobileDetail(false)} onClose={closeDetail} renderStopControl={renderStopControl} />}
  </div>;
}

function SymbolDetail({ onIntentChange, group, th, mobile, onBack, onClose, renderStopControl }: { onIntentChange?: (key: string, intent: PositionIntent) => void; group: SymbolGroupViewModel; th: typeof THEMES[Theme]; mobile: boolean; onBack: () => void; onClose: () => void; renderStopControl?: (position: Position) => ReactNode }) {
...
interface ManagementActionProps {
  /** POSITION-INTENT-0001: saves the one stored intent for a position; the control is hidden when absent. */
...
function AnalysisView({ model, th, getManagementActions, onExecute, renderStopControl, onAnalyze, renderAnalysisConversation, onFindPmccShortCall, onIntentChange, sellDeps }: { model: PositionsWorkspaceModel; th: typeof THEMES[Theme]; sellDeps?: SellStockDialogDeps } & ManagementActionProps) {
  ...
  // The Stock holdings table below takes its column widths from this table, so the two line up.
  const tableRef = useRef<HTMLTableElement>(null);
  const [measuredWidths, setMeasuredWidths] = useState<Record<string, number> | null>(null);
  useEffect(() => {
    const table = tableRef.current;
    if (!table) return;
    const measure = () => {
      const next: Record<string, number> = {};
      table.querySelectorAll('th[data-column-id]').forEach(header => { next[header.getAttribute('data-column-id') as string] = header.getBoundingClientRect().width; });
      setMeasuredWidths(previous => (sameWidths(previous, next) ? previous : next));
    };
    measure();
  ...
  return <>
    ...
    <div className="max-w-full overflow-x-auto rounded-xl border border-white/10" tabIndex={0} aria-label="Position analysis table, horizontally scrollable"><table ref={tableRef} className="min-w-max border-collapse text-left text-[11px]"><thead><tr>{ANALYSIS_COLUMNS.filter(column => columns.includes(column.id)).map(column => {
...
function AnalysisRow({ onIntentChange, position: p, columns, th, actions, onExecute, renderStopControl, onAnalyze, savedNote, onSaveNote, savedAlert, onSaveAlert, chartOpen, setChartOpen, sparkData, setSparkData, sparkLoading, setSparkLoading }: { onIntentChange?: (key: string, intent: PositionIntent) => void; position: Position; columns: AnalysisColumnId[]; th: typeof THEMES[Theme]; actions: ActionType[]; onExecute?: (position: Position, action: ActionType, initialRollMode?: 'close' | 'roll') => void; renderStopControl?: (position: Position) => ReactNode; onAnalyze?: (position: Position) => void; savedNote: string; onSaveNote: (position: Position, note: string) => Promise<void>; savedAlert: { targetPrice: number; direction: 'above' | 'below' } | null; onSaveAlert: (position: Position, targetPrice: number | null, direction: 'above' | 'below') => Promise<void>; chartOpen: boolean; setChartOpen: (open: boolean) => void; sparkData: number[] | null; setSparkData: (data: number[] | null) => void; sparkLoading: boolean; setSparkLoading: (loading: boolean) => void }) {
  ...
  const earningsLine = nextEarningsLine(p.nextEarningsDate, p.nextEarningsEstimated, p.expDate);
  const extrinsic = buildExtrinsicView(p);
  ...
  const cell: Record<AnalysisColumnId, ReactNode> = {
    identity: <><b className="text-white">{p.symbol}</b><span className="block text-amber-300">{standaloneLeap ? 'LEAPS CALL' : p.strategy}</span><span className={th.textFaint}>{standaloneLeap ? 'Standalone · ' : ''}{p.quantity} contract{p.quantity === 1 ? '' : 's'}</span><span className="mt-1 block"><IntentSelect position={p} onIntentChange={onIntentChange} /></span><span className="mt-1 block"><ChartLinkButton symbol={p.symbol} chartSymbol={INDEX_CHART_SYMBOLS[p.symbol.toUpperCase()] ?? p.symbol} instanceKey={p.key} th={th} showChart={chartOpen} setShowChart={setChartOpen} sparkData={sparkData} setSparkData={setSparkData} sparkLoading={sparkLoading} setSparkLoading={setSparkLoading} /></span>{earningsLine && <span className={`mt-1 block ${earningsLine.beforeExpiry ? 'text-amber-300' : th.textFaint}`} title={earningsLine.beforeExpiry ? 'Earnings fall on or before this position expires' : 'Next earnings date; it falls after this position expires'} data-testid="next-earnings">{earningsLine.text}</span>}</>,
    ...
    strike: <><span className="block">{p.legs.map(leg => `${leg.direction === 'Short' ? 'Short ' : 'Long '}${leg.strikePrice}${leg.optionType}`).join(' · ') || '—'}</span><span className={`mt-1 block ${breakeven.values.length ? 'text-white' : th.textFaint}`} title={breakeven.unavailableReason ?? undefined}>{breakeven.values.length ? `AT-EXP B/E ${breakeven.values.map(value => moneyExact(value)).join(' / ')}` : 'AT-EXP B/E —'}</span>{priceBuffer != null && <span className={`block ${SEMANTIC_TONE_CLASS[priceBuffer >= 0 ? 'positive' : 'negative']}`}>Price Buffer {priceBuffer >= 0 ? '' : '−'}{money(Math.abs(priceBuffer))}{priceBufferPct != null ? ` (${priceBufferPct.toFixed(1)}%)` : ''}</span>}{extrinsic && <><span className={`mt-1 block ${th.textMuted}`} data-testid="extrinsic-now" title="Time and volatility value in the option price: the option price minus how far it is in the money">Extrinsic {moneyExact(extrinsic.now)}{extrinsic.pctOfValue != null ? ` (${extrinsic.pctOfValue.toFixed(0)}%)` : ''}</span>{extrinsic.atEntry != null && extrinsic.change != null && <span className={`block ${th.textFaint}`} data-testid="extrinsic-was">was {moneyExact(extrinsic.atEntry)} at entry ({extrinsic.change >= 0 ? '+' : '−'}{moneyExact(Math.abs(extrinsic.change))})</span>}</>}</>,
  ...
  return <tr className="align-top hover:bg-white/[0.03]">{ANALYSIS_COLUMNS.filter(column => columns.includes(column.id)).map(column => <td key={column.id} className={`max-w-64 border-b border-r border-white/10 px-3 py-3 ${th.textMuted} ${column.id === 'identity' ? `sticky left-0 z-10 ${th.bg}` : ''} ${column.id === 'strike' ? 'w-24 max-w-24 whitespace-nowrap' : column.id === 'underlying' ? 'w-20 max-w-20 whitespace-nowrap' : column.id === 'orders' || column.id === 'notes' ? 'w-40 max-w-40' : ''}`}>{cell[column.id]}</td>)}</tr>;
}
