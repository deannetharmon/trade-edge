...
function buildIncomeOpportunities(input: PositionsWorkspaceInput): ExistingIncomeOpportunity[] {
  ...
  for (const position of options) {
    ...
    const base = {
      ...
      heldPmccLong: exactContract ? { expiration: position.expDate, dte: position.dte, strike: leg.strikePrice, quantity: position.quantity, entryDebitPerShare: leg.avgOpenPrice, markPerShare: leg.currentPrice, delta: leg.currentDelta, stockPrice: position.stockPrice } : undefined,
    };
