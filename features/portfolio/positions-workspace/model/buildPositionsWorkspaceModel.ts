...
function buildIncomeOpportunities(input: PositionsWorkspaceInput): ExistingIncomeOpportunity[] {
  ...
  for (const position of options) {
    ...
    const base = {
      ...
      heldPmccLong: exactContract ? { expiration: position.expDate, dte: position.dte, strike: leg.strikePrice, quantity: position.quantity, entryDebitPerShare: leg.avgOpenPrice, markPerShare: leg.currentPrice, delta: leg.currentDelta, stockPrice: position.stockPrice, nowIvr: position.ivr ?? null, atEntry: { capturedAt: position.entrySnapshotCreatedAt ?? null, entryDate: position.entryDate ?? null, stockPrice: position.stockPriceAtEntry ?? null, deltaPerShare: position.deltaAtEntry != null && Math.abs(position.quantity) > 0 ? position.deltaAtEntry / Math.abs(position.quantity) : null, ivr: position.ivrAtEntry ?? null }, history: (position.snapshotHistory ?? []).slice(-120).map(item => ({ date: item.date, currentValue: item.currentValue ?? null, netDelta: item.netDelta ?? null, stockPrice: item.stockPrice ?? null })) } : undefined,
    };
