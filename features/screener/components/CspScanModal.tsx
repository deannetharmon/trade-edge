  ...
  return (
    ...
    >
      <div ref={contentRef} className="flex flex-col gap-0">
        ...
        <fieldset className="mt-5 rounded-lg border border-neutral-800 bg-neutral-900/60 p-3 text-[10px] text-neutral-300"><legend className="px-1 text-xs font-bold text-neutral-300">Capital filter</legend><label className="flex items-center gap-2"><input type="checkbox" checked={request.affordableOnly} onChange={event => updateDraft(prev => ({ ...prev, affordableOnly: event.target.checked }))} />Only show CSPs affordable with available capital</label>{request.affordableOnly && <label className="mt-3 flex flex-col gap-1">Cash limit per CSP (optional)<input aria-label="Cash limit per CSP" type="number" min="0" placeholder="Use available account cash" value={request.capitalLimit ?? ''} onChange={event => updateDraft(prev => ({ ...prev, capitalLimit: event.target.value === '' ? null : Number(event.target.value) }))} className="w-48 rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-white" /></label>}<p className="mt-2 text-neutral-400">A CSP requires strike × 100 × contracts in collateral. Leave this blank to use available cash in your selected account. Enter a lower amount to limit each trade.</p></fieldset>
