...
const PRESETS: Array<{ key: string; label: string; description: string; rules: CspRulesType }> = [
  ...
  { key: 'opportunity', label: 'More opportunities', description: 'Wider delta and DTE search with OI still advisory.', rules: { IVR_MIN: 20, IVR_MAX: 75, DELTA_MIN: 0.12, DELTA_MAX: 0.30, DTE_MIN: 21, DTE_MAX: 60, OI_MIN: 250, BID_ASK_MAX: 0.10 } },
];
    ...
    >
      <div ref={contentRef} className="flex flex-col gap-0">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {([['DTE_MIN','Min DTE'],['DTE_MAX','Max DTE'],['DELTA_MIN','Min delta'],['DELTA_MAX','Max delta'],['OI_MIN','Preferred OI']] as Array<[keyof CspRulesType,string]>).map(([key,label]) => <label key={key} className="text-[10px] text-neutral-400">{label}<input aria-label={label} type="number" step={key.includes('DELTA') ? '0.01' : '1'} value={request.rules[key]} onChange={e => setRule(key, Number(e.target.value))} className="mt-1 w-full rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-xs text-white" /></label>)}
