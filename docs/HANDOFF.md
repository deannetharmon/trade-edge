...
Last updated: 2026-07-16 (post PI-0012A Portfolio Review Composition Layer). Paste this file (or point Claude at its repo path) at the start of a new chat to resume with full context.
...
- Latest local commit: `fddb3a8` — "feat(portfolio): add portfolio review composition layer" (PI-0012A)
- Previous local commit: `fcaebce` — "docs: add UX Polish implementation report, update handoff"
- Latest known remote commit: `5cc82c5` — "docs: add session handoff for portfolio intelligence" (Dean committed/pushed this + `e35c96d` + `2ee4b0e` + `fcaebce` himself after prior sessions; none of those have been pushed from this sandbox). Local sandbox has no git network credentials, so this couldn't be pushed from here either; Dean pushes from his own machine:
  ```
  git fetch
...
- `app/portfolio/page.tsx` — main portfolio dashboard page, wires most of the above together (large file); also home to `buildPositionPrompt()` (AI Analysis prompt), `AnalysisPanel` (AI Analysis UI, separate from Position Intelligence), and the `portfolioReviewInput`/`portfolioReview` `useMemo` pair feeding `<PortfolioReviewCard>`.
...
- `features/portfolio/review/PortfolioReviewCard.tsx` — the Portfolio Review UI card, first section on the Portfolio page's Positions tab.
