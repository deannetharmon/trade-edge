...
| 3 | ~~Confirm audit trail DB~~ — **Resolved:** it's Redis via `ioredis`/`REDIS_URL` (same connection as portfolio/trade-log/decision-log), not Postgres. New `auditTrailStore.ts` follows `decisionLogStore.ts`'s capped-list pattern. | This session | Resolved 2026-07-11 | — |
...
## Status
Last updated: 2026-07-11 — open items: 1
