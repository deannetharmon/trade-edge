...
9. **`PortfolioModeIndicator` is NOT reactivated in this ticket, correcting this document's original plan.** An earlier revision of this section planned to reactivate the global LIVE/PAPER switch once `/dashboard` and `/portfolio` were self-gating, on the premise that those were the only remaining live-portfolio-dependent screens. That premise was checked directly against the application and found incomplete: `app/engine/page.tsx` (Income Engine) independently fetches and renders real live positions and account balances via its own `ttFetch` calls, entirely outside `PortfolioDataProvider`, and never calls `usePortfolioMode()`; `app/rinse-repeat/page.tsx` independently fetches live account data and submits a real live order (`ttPostComplex` OTOCO, already flagged in `docs/reviews/ES-0002-Broker-Submission-Inventory.md` item 11) with the same zero mode-awareness. Reactivating the global indicator today would let a user select PAPER, see that reflected globally, and still have `/engine` display real live capital/positions and `/rinse-repeat` remain able to submit a real live order — reintroducing the exact ambiguous-context defect PT-0002A's corrective round existed to prevent, relocated to two screens this ticket does not touch. See §3.4.

### 2.3 Out of scope (unchanged from PT-0002A, still deferred)

Autopilot activation, new Decision/Opportunity Engine rules, new PAPER strategy/order-entry logic, Capital Allocation Engine, Income Engine, multi-portfolio support, copying LIVE positions into PAPER, automatic LIVE/PAPER synchronization, screen redesigns beyond the mode-gate itself, broker submission semantic changes. `app/rinse-repeat/page.tsx`'s separate OTOCO path (ES-0002 broker inventory item 11) and `app/engine/page.tsx`'s independent live account-data fetch are both untouched — separate tickets' candidates (ES-0003, and a not-yet-scoped Income Engine mode-awareness ticket, respectively), not this one's. Both remain directly relevant to, and are the reason for, §2.2 item 9's decision not to reactivate the global mode indicator in this ticket.
...
An earlier revision of this document planned to reactivate `PortfolioModeIndicator`'s disabled PAPER control and remove its full-screen PAPER block in this ticket, on the premise that once `/dashboard` and `/portfolio` were self-gating, no screen could display an ambiguous LIVE/PAPER mismatch (PT-0002A §9.1 identified that exact failure mode: a trader selects PAPER, sees the badge, and is still looking at real LIVE data). That premise was checked directly against the application surface and found incomplete (§2.2 item 9): `app/engine/page.tsx` and `app/rinse-repeat/page.tsx` both independently display, and for `/rinse-repeat`, act on, real live account data with no `PortfolioMode` awareness at all — neither imports `usePortfolioMode()`. Reactivating the indicator today would reintroduce PT-0002A's original ambiguous-context defect on those two screens.
...
Reactivation should be revisited only after `/engine` and `/rinse-repeat` gain equivalent mode-awareness, or a full application-wide inventory confirms no further live-data surface remains ungated.
