# Launch-Gate Record — `pmcc_deep_analysis` — v1

**Status:** DRAFT
**Route:** `pmcc_deep_analysis`
**Owner:** <n> (process owner: Paul) · **Approver:** <n> (Dean) · **Approved on:** <date> · **Expires / re-review by:** <date>
**Registry entry:** `lib/ai-policy/launchGates.ts` id `pmcc_deep_analysis-v1` (approval = reviewed commit adding this entry; **not added**)
**Model tier / model / provider policy version under test:** reasoning / Not yet named (decision D3, reasoning tier) / <provider policy version>
**Prompt template + lexicon + registry versions under test:** Not yet built: prompt template, registry and fixtures arrive with AI-POLICY-0001E (after the PMCC three-stage discovery flow lands; needs D15)
...
Route not built yet. This draft holds the place; it cannot be approved until the route, its fixtures and its reviewer sample exist.
