# Launch-Gate Record — `grounded_chat` — v1

**Status:** DRAFT
**Route:** `grounded_chat`
**Owner:** <n> (process owner: Paul) · **Approver:** <n> (Dean) · **Approved on:** <date> · **Expires / re-review by:** <date>
**Registry entry:** `lib/ai-policy/launchGates.ts` id `grounded_chat-v1` (approval = reviewed commit adding this entry; **not added**)
...
**Prompt template + lexicon + registry versions under test:** Prompt template `grounded-chat` `tmpl-v1-draft`; lexicon `lex-v1-draft`; validator `val-v1`; citable-field registry `lib/ai-policy/registries/scanSession.ts` (shared with scan_summary; draft field list awaiting Ian)
...
Built in AI-POLICY-0001B. Offline fixtures: `lib/ai-policy/eval/fixtures/grounded_chat/` (includes prompt-injection attempts). Shares the IV-rank known issue with scan_summary.
