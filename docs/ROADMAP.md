...
- Last merged: CI-FLAKY-0001 (`25821b7`, warm-up `beforeAll` in three first-test cold-start screener tests; cause inferred, not reproduced on the two named tests), SCAN-EXPORT-0001 scan PDF export (Dean's own work, `1689c98`), EARNINGS-PRECHECK-0001 guard test and Ian's ruling wording (`5483462`).
- **EARNINGS-PRECHECK-0001 close-out (P1):** live; Quinn's CSP path confirmed on main; Ian ruled 2026-09-24 that a card whose in-window expiries are all on or after earnings counts as an earnings fail. Open: a whole-page test that such a card offers the post-earnings calendar re-screen button did not find the button, so it is unverified whether the advisory reaches `failReasons` (page.tsx ~:1490, counted by hasEarningsBlock, SmartSuggestions and the calendar button, all matching the substring "Earnings"). Also owed: Diane to make the row severity consistent with the fail count, one clean full-suite run.
- **ES-0003 rinse-repeat OTOCO entry guard:** implemented by Dean (`d37d73f`, with `otocoEntrySafety.test.ts` and a review doc); needs Quinn's verification note and Ian/Alan review (live-order path).
- **LCC-0001A Gate A:** acceptance evidence defined (`8891add`); blocked on a real Covered Call shadow-parity sample (active account holds no stock; no synthetic evidence allowed). Do not start LCC-0001B before Gate A.

## Next (recommended order; scoped or ready to scope)

0. **EARNINGS-DATEBASIS-0001 (P1, ruled by Ian 2026-09-24, after the small-ready bundle):** shared `daysUntil` (UTC-midnight rounding) makes earnings day read "Already reported" after ~12:00 UTC so the checklist skips the fail/warn (a qualified trade can sit through an earnings-day print with a green check); same bug in the expiry filter and rinse-repeat; fix ~11 sites on the New York basis (`daysUntilNy`). Autopilot `recommendationEngine.ts:147` is its own reviewed item (order path). Alan fixtures, Quinn review.
