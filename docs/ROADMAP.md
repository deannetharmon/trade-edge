...
Open items:
- **ES-0003 OTOCO guard:** verified with gaps (Quinn); tests and dead-code removal done. Still open for Ian: the multi-leg close uses `order-type: Stop`, but CLAUDE.md says stop-market is unsupported on multi-leg positions; unproven whether TastyTrade accepts it. Ticket status stays "pending review and validation" until Ian rules.
