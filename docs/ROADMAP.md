...
- EARNINGS-PRECHECK-0001 (CC and CSP earnings pre-check becomes advisory and per-expiry; Dean's false earnings warning): merged as `1abbcbdf` and live in Production. Focused CC/CSP/date tests and type check pass; Vercel's clean production build passed in 56s (39s post-build). **Quinn review: conditional hold, not yet accepted.** Before acceptance, add a checklist-level regression proving a CC with no eligible candidate shows the earnings advisory rather than an earnings failure, then obtain one clean completion of the full test suite. CSP per-expiry classification and New York-date coverage are approved by review.
- CI-FLAKY-0001 (test-only): stabilize `ScreenerUXHierarchy` and `ScreenerSessionWiring` "accounting reconciliation", both load-flaky in full-suite runs. Ticket saved; Dane building. Also open: Ian to rule whether the "no eligible expiration" advisory copied into `failReasons` (page.tsx ~:1490, counted by hasEarningsBlock, SmartSuggestionsPanel and the calendar-button block) is intended; Quinn to confirm the CSP per-expiry classification path. Then EARNINGS-PRECHECK-0001 is accepted.
