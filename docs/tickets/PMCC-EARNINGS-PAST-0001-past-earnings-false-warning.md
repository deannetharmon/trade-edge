...
6. **Build condition: closed (Dean, 2026-09-24).** Ian asked for the real shape of the earnings value reaching `resultForPair`. Real samples from a live `market-metrics` response across 15+ symbols (AAPL, AMD, AMZN, APP, BX, DDOG, GM, GOOGL, META, MRVL, MSFT, MU, ORCL, PEP, PLTR, TER, TSLA) show `expected-report-date` is always a plain date-only string such as `"2026-10-29"`: no time, no timezone, no padding. The grammar assumption is confirmed against real data, and the date-only form is in the accepted grammar. **The sample did not include an already-reported date, so the golden fixture for the past-date case (B1, B2) is still synthetic, not drawn from a captured live value.** If a different shape ever appears (space separator, lowercase `t`/`z`, padding), widen the grammar or normalize upstream; do not suppress the gate, and do not add a junk-value warning here (`EARNINGS-NO-DATE-0001` owns that gap).
