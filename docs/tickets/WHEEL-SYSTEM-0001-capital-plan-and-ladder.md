...
- **O8** Months to unlock (Dean 2026-09-26: no new capital, only reinvested returns). Add an editable assumed monthly growth of the account (default 0.75%, about 9% a year; stated as no losses, fees or taxes) and a "months to unlock" column: `months = ceil(ln(fitsAtAccountSize / A) / ln(1 + g))`, blank when the name already fits, "not reached" if `g <= 0`. Illustrative results at A = 50,000: XLV (53,000) 8 months and PLTR (58,700) 22 months at g = 0.72%. Alan to fix rounding and fixtures. Note the Concentrated profile lowers every threshold by one quarter (cash / 0.40).
