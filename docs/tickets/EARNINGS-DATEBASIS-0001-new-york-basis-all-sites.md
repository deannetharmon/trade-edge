...
**Ian's precedents (2026-09-24):** missing or zoneless `asOf` (T unknown) fails toward at-risk/exclusion (as slice D); earnings on the scan day (0 days) counts as at-risk even if reported pre-market; a timestamp-suffixed earnings date uses its leading date; expiry-window membership stays on the old basis. **Short-expiry alert (Ian, 2026-09-24): use NY-day counting.** At 8pm ET on X-1 the call really expires tomorrow, so a warning at dte 1 is correct; expiry-day and expired states stay critical (dte <= 0 on the NY day). Apply the same NY "today" to the ex-dividend and earnings alerts in `pmccLifecycle.ts` so all three share one basis. Tests: pin now 2026-09-25T00:00Z with X 2026-09-25 to warning at dte 1, and X 2026-09-25 with NY date 09-25 to critical; change the message "1 days" to "1 day" if free. All four precedents confirmed. **Note in the roadmap: earnings/expiry-inclusion math (NY basis) and DTE-window membership (old basis) will temporarily differ; DTE gets its own ticket.**
