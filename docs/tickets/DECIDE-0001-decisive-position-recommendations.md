...
**Status:** REVISION 7 (2026-09-26). **Not approved to build.** Every gate has now reviewed the design (Ian, Alan, Quinn, Paul, Diane, and Dane, the developer, G6), and their required changes are folded in here; the mock (revision 6, link below) shows the result. **Dane's verdict: only S3-0a (the false-"Hold" fix) can start, and only after Dean's decision 7 and two small answers (below); every other slice has a named blocker.** What remains: Dean's decisions (see "Decisions Dean owes"), Ian's remaining rulings, Alan's fixtures, and the answers Dane lists per slice. **Approved so far (Paul): Phase 0 and 0001A, and the display-only safety slice S3-0.** Everything else is approved slice by slice.
...
| **Dane (developer)** | Reviewed revision 6 (2026-09-26): S3-0a ready after two answers; every other slice has a named blocker; his 15 corrections are folded into this revision. Next: confirm each slice's inputs before he starts it (table in his report, summarized under "What Dane needs before each slice") | Any development | **Done for revision 6; re-check on the slice he starts** |

**Nothing past Phase 0, 0001A and S3-0 is built until every row above is done.**

## Problem

1. **One live recommendation engine drives everything the app shows:** `scorePortfolioPositionObjective` (`lib/portfolio-data/acquisition.ts:213`) calls `evaluatePositionObjective` and `selectManagementIntent` (`lib/portfolio-intelligence`); its result is `pos.recommendation` (and `pos.portfolioObjective`). **A second call to the same engine** feeds Today's Priorities, the briefing and the command center (`portfolioIntelligenceAdapter.ts:154-172`, `dashboardComposition.ts:194-218`). `getRecommendation` (`acquisition.ts:2228`) is dead code (only tests call it). *History:* on 2026-09-25 (ACQUIRE-WHEEL-LIVE-0001) the live engine began honoring Acquire and Wheel for a lone short put (no loss exit, no 21-DTE roll suggestion, a plain context line), with 14 tests in `assignmentPlannedRecommendation.test.ts`; S2's shadow comparison starts from that behavior.
...
- **Dead code:** `getRecommendation` and its test blocks are deleted only after the cutover. In `stopLossWiring.test.ts` the unchanged blocks are `:99`, `:214`, `:314`, `:328`, `:367`; every block that calls `getRecommendation` (`:191-213`, `:387`, `:519`, `:624-703` with calls at `:674` and `:698`, `:704`, `:733-770` with calls at `:752` and `:761`, and the MU production-incident fixture `:780-970`, calls from `:861`) is **ported to `PositionDecision` with the same numbers; none is deleted** (the trust-boundary guard from TE-0002 round 3).
...
| **S3-0a** No false "Hold" (**first slice; small; ready after two answers**) | `PositionsWorkspace.tsx:569` and `:747-748`: a missing recommendation shows "No recommendation" and "Recommendation could not be calculated. Refresh." with a per-row named Refresh button; `recommendationTone` returns `'neutral'` for it; no chart fetch is added. **Answers needed first:** the Refresh button calls the page's existing portfolio refresh (a new `onRefresh` prop wired at the `<PositionsWorkspace>` call site in `app/portfolio/page.tsx`; **proposal for Dean to confirm**); and `recommendationTone` returns `'warning'` for act-now labels and `'neutral'` otherwise, without changing `SEMANTIC_TONE_CLASS.negative` (`model/presentation.ts:7`, shared with P&L; **proposal for Diane to confirm**) | Dean's decision 7 |
| **S3-0b** Contradicting banners deleted (small) | Delete the CSP 21-DTE banners (`page.tsx:8258-8283`), the duplicate "PROFIT TARGET HIT" (`:8252-8256`) and `getExtendSignal` (`:1735-1752`, caller `:8909`); `isShortDateEntry` stays | S3-0a, Dean's decision 2 |
| **S3-0c** Red removal on the existing labels (medium to large; **one named list of files and lines per push**) | Red replaced on the badge, panel chip and border, the red card border (`page.tsx:8221`), `dteColor`, "✕ None" and the Cut Losses buttons (the rename to "Close at a loss" is a separate copy change; workspace tests assert the button name at `:96-101`). **Priority lists, mission control and the health surfaces are S3-D, not S3-0.** | S3-0b, Diane's per-surface replacement for red |
| **S3-1** Bought options and `needsClose` (live-engine logic; **Ian approves first and approves the affected-position list, produced by the same debug page as S2; full suite**) | Bought options show "Not judged yet" instead of Take Profit or Cut Losses; `needsClose` is redefined for spreads and iron condors only, so covered calls, CSPs and bought options no longer show the red CLOSE NOW banner | S3-0, Dean's decision 2 |
| **S2** `PositionDecision` pure module, shadow mode | Nothing on screen for positions; a signed-in debug page `/debug/decisions` prints the disagreement table for Dean to copy (the browser makes the broker calls; the pure module is bundled with the page, so no console script or local Node is needed); Ian approves the list | S1, Dean's decisions 3, 5 |
...
- **The trader's own profit target** (`pos.profitTarget`, `hitTarget`, used at `app/portfolio/page.tsx:8219` and `:8252`) is what every "profit" rule reads; 50% is the default when none is set. Fixtures run at 50% and at one non-default target.
...
- **Paths:** bare filenames in this ticket are `features/portfolio/positions-workspace/PositionsWorkspace.tsx`, `features/portfolio/components/PositionHealthBadge.tsx`, `features/portfolio/components/PositionRecommendationBadge.tsx`, and `lib/portfolio-intelligence/adapters/portfolioIntelligenceAdapter.ts` (a second file of that name in `lib/paper-trading/adapters/` is not the one).
...
- **Endpoint contract (proposal for Quinn to confirm before S0 starts; the ticket must fix these):** `GET /api/chart/history?symbol=X` returning `{ bars: [{ t, o, h, l, c, v }], asOf }`, signed-in only, a per-user limit of 30 requests a minute, cached per symbol until the next NY session close; the entry support level is stored write-once in Redis under `entry-support:{userId}` keyed by position key with `{ level, asOf, basis, source }`; split detection reads Yahoo `events=splits` on the same request.
...
- **Decision log finding:** `lib/decision-review` holds 0 records because records are created only when the trader opens the section and saves (`DecisionReviewSection.tsx:84` → `app/portfolio/page.tsx:10349` → `/api/decision-reviews`). Nothing auto-writes. There is nothing to investigate.
...
New pure module `lib/market-intelligence/brokenStock.ts`. It changes **no recommendation and no action**. **Where it runs:** the history fetch is in a page-level data hook that passes the results to the workspace as props; `PositionsWorkspace` itself never fetches `/api/chart` (its test `:363` asserts that). Where it shows: one calm note on Hold, the mock's state S1 ("Broken-stock check: trend, gap clear; support broken on high volume.").
...
*Legend wording: "Amber edge: act now". Ian to sign off that in-the-money expires soon is an act-now state.* **`recommendationTone` (Dane):** returns `'warning'` for act-now labels and `'neutral'` otherwise; `SEMANTIC_TONE_CLASS.negative` is unchanged because P&L and price comparisons use it. In the Position Intelligence panel the urgency chip and border are replaced by the dot table above; the urgency values high and critical no longer style anything red.

**Amber budget:** amber text appears only on true warnings (a loss stated on an Acquire position, "stop cannot trigger", earnings, buffer under 5%); neutral information (working order, let-it-expire, short-dated entry, delta note) is dim gray; "Rule met" is always gray.
...
13. **Text limits (reason 110, changes-if 72, notes 72 in total, numbers 90) hold for every state.** The mock's `S` array passes them (all 39 states, checked by a script; re-run whenever the mock changes) and a test enforces them on the product's copy.
...
| G6 | Dane (developer) | Reviewed revision 6: S3-0a ready after two answers; other slices not ready (blockers named); corrections folded into revision 7 |
